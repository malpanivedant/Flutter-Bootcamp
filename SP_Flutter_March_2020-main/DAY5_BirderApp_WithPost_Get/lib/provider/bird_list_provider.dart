...
class BirdListProvider extends ChangeNotifier {
  ...
  void addNewBirdToList(BirdModel _birdToBeAdded) async {
    // add a new bird here //  notifyListeners();
    // add this to the list of birds only when server returns success message !
    var responseStatus = await BirdService.addBirdToFirebaseDB(_birdToBeAdded);
    // before adding something to the list check if db insertion is successful
    if (responseStatus == "Success") {
      listOfBirds.add(_birdToBeAdded);
      notifyListeners();
    }
  }

  void getAllBirds() async {
    listOfBirds = await BirdService.fetchBirdsFromDB();
    notifyListeners();
  }
